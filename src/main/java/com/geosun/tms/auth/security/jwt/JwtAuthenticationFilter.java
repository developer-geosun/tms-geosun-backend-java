...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (user.isDeleted()) {
            SecurityErrorWriter.writeJson(response, objectMapper, HttpServletResponse.SC_FORBIDDEN,
                    "Forbidden", "USER_DELETED", "User account has been deleted", request.getRequestURI());
            return;
        }
        if (!user.isActive()) {
            SecurityErrorWriter.writeJson(response, objectMapper, HttpServletResponse.SC_FORBIDDEN,
                    "Forbidden", "ACCOUNT_DISABLED", "Account is disabled", request.getRequestURI());
            return;
        }
    ...
    private void writeUnauthorized(HttpServletResponse response, HttpServletRequest request, String message)
            throws IOException {
        SecurityErrorWriter.writeJson(response, objectMapper, HttpServletResponse.SC_UNAUTHORIZED,
                "Unauthorized", "UNAUTHORIZED", message, request.getRequestURI());
    }

    private void writeInvalidSession(HttpServletResponse response, HttpServletRequest request) throws IOException {
        SecurityErrorWriter.writeJson(response, objectMapper, HttpServletResponse.SC_UNAUTHORIZED,
                "Unauthorized", "INVALID_SESSION", "Refresh session is invalid or expired", request.getRequestURI());
    }
}
